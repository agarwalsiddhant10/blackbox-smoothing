...
class AttackerModel(ch.nn.Module):
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
                fake_relu=False, no_relu=False, with_image=True, should_normalize = True, **attacker_kwargs):
        """
        ...
        if make_adv:
            ...
            self.eval()
            adv = self.attacker(inp, target, should_normalize = should_normalize, **attacker_kwargs)
            if prev_training:
                self.train()
