...
def load_custom_model(model, checkpoint_path, dict_key, parallel = False):
    """
    Creates a custom model and loads a saved checkpoint

    Args:
        model (torch.nn.Module): a torch model 
        checkpoint_path (str): the path to the saved checkpoint
        dict_key (str): the key to access the model state_dict 
    Returns: 
        The Attacker Model
    """
    # optionally resume from a checkpoint
    ...
    if checkpoint_path:
        if os.path.isfile(checkpoint_path):
            print("=> loading checkpoint '{}'".format(resume_path))
            checkpoint = ch.load(checkpoint_path, pickle_module=dill)
            
            # Makes us able to load models saved with legacy versions
            state_dict_path = dict_key
            # if not ('model' in checkpoint):
            #     state_dict_path = 'state_dict'
            ...
            error_msg = "=> no checkpoint found at '{}'".format(checkpoint_path)
            raise ValueError(error_msg)
